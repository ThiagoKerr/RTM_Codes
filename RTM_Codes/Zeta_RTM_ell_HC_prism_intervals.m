...
densidade(densidade==-99999000)=densidade_media; densidade(densidade==-3.40282e+38)=densidade_media;
...
% Resolução
res=0.0002777777777777779402; res_rad=res*rad;             % Resolução em ° e em read, respectivamente
radius_metros_intervalo=1000; radius_metros_total=210000; % m
...
minlat=-13.2468055555555431*rad-(res_rad/2);    % latN
minlon=-50.5781944444444491*rad+(res_rad/2);   % longW
...
V(1:length(xx1),coluna)=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,densidade);  % m
else
V(1:length(xx1),coluna)=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,rho);  % m
end
end
Eta_RTM_prisma=sum(V,"omitnan")/gama+Eta_RTM_HC_Omang   % m
toc

% Distúrbio da gravidade por fatias de raios para geração de gráfico
dg_RTM_prisma_raios=cumsum((sum(V,"omitnan")).*1D5)+Eta_RTM_HC_Omang;   % m
