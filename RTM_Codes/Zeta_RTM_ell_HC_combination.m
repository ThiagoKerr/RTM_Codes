...
radius_metros_poliedro=0.02*3600*30; radius_metros_prisma=0.03*3600*30; radius_metros_tesseroide=0.15*3600*30;
radius_poliedro=radius_metros_poliedro/(3600*30);           % raio de integração em ° para poliedro (considerando 1"=30 m e 1°=108000 m) - 2,34 km
...
clear sin_lat_o_poliedro_NW sin_lat_o_poliedro_NE sin_lat_o_poliedro_SW sin_lat_o_poliedro_SE Nell_NW Nell_SE Nell_NE Nell_SW
...
V_poliedro=G.*densidade_o_poliedro.*g(1:size1,1);
else
V_poliedro=G.*rho.*g(1:size1,1);
end

Eta_RTM_poliedro=(-sum(V_poliedro,"omitnan")./2)./gama   % m
...
V_pp=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,densidade_o_pp);  % m/s2
else
V_pp=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,rho);  % m/s2
end
Eta_RTM_pp=(sum(V_pp,"omitnan")./2)./gama   % m
...
% Raio de integração
lo(lo>radius_metros_prisma)=NaN; lo(lo<=radius_metros_poliedro)=NaN; index=~isnan(lo); index=double(index); index(index==0)=NaN;
...
V_prisma=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,densidade_o_prism);  % m/s2
else
V_prisma=Prism_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,rho);  % m/s2
end
Eta_RTM_prisma=sum(V_prisma,"omitnan")./gama   % m
...
lo_tesseroide=real((r.*r+ro.*ro-2.*r.*ro.*cos_psi_o).^0.5);
...
K000=ro_2.*cos_lat_o./lo_tesseroide;
K002=-(r.*ro_3.*cos_lat.*cos_lat_o.*cos_lat_o)./lo_tesseroide_5.*(lo_tesseroide_2.*cos_dlong-3.*r.*ro.*cos_lat.*cos_lat_o.*sin_dlong.*sin_dlong);
K020=(ro_2./lo_tesseroide_5).*(-cos_lat_o.*(r_2+ro_2).*(r_2+ro_2-r.*ro.*sin_lat.*sin_lat_o)+r_2.*ro_2.*cos_lat_o.*(sin_lat.*sin_lat.*(3-sin_lat_o.*sin_lat_o)-cos_lat.*cos_lat.*(2-sin_lat_o.*sin_lat_o).*cos_dlong.*cos_dlong)+ ...
      +r.*ro.*cos_lat.*(3-sin_lat_o.*sin_lat_o).*(r_2+ro_2-2.*r.*ro.*sin_lat.*sin_lat_o).*cos_dlong);
K200=(r_2.*cos_lat_o./(lo_tesseroide_5)).*(2.*lo_tesseroide_2-3.*ro_2.*sin_psi_o.*sin_psi_o);

if strcmp('sim',modelo_densidade)
V_tesseroide=densidade_o_tesseroide.*cte.*(K000+1/24.*(K200.*delta_r.^2+K020.*delta_lat.^2+K002.*delta_long.^2));  % m2/s2
else
V_tesseroide=rho.*cte.*(K000+1/24.*(K200.*delta_r.^2+K020.*delta_lat.^2+K002.*delta_long.^2));  % m2/s2
end
Eta_RTM_tesseroide=sum(V_tesseroide,"omitnan")./gama   % m

clear lo index x y z sin_lat_o cos_lat_o sin_long_o cos_long_o sin_dlong cos_dlong r1 r2 delta_r ro cos_psi_o sin_psi_o L000 cte
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%% Cálculo dos distúrbios de gravidade RTM no entorno da estação de cálculo %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PONTO MASSA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Raio de integração
lo(lo<=radius_metros_tesseroide)=NaN; lo(lo>radius_metros)=NaN; index=~isnan(lo); index=double(index); index(index==0)=NaN;
...
lo_ponto_massa_2=lo_ponto_massa.*lo_ponto_massa;
lo_ponto_massa_3=lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa;
lo_ponto_massa_4=lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa;
lo_ponto_massa_5=lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa;
lo_ponto_massa_7=lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa.*lo_ponto_massa;
...
Eta_RTM_ponto_massa=sum(V_ponto_massa,"omitnan")./gama   % m
...
Eta_RTM_total=  Eta_RTM_poliedro  +  Eta_RTM_pp  +  Eta_RTM_prisma  +  Eta_RTM_tesseroide  +  Eta_RTM_ponto_massa  %+  Eta_RTM_HC_Omang   % mGal
...
V_todos=[-V_poliedro./2; V_pp./2; V_prisma; V_tesseroide; V_ponto_massa];

Eta_RTM_todos_raios=zeros(1,radius_metros/1000);
radius_grafico=zeros(1,radius_metros/1000);
for i=1:radius_metros/1000
lo_todos_grafico=lo_todos; lo_todos_grafico(lo_todos_grafico>(i*1000))=NaN;
index=~isnan(lo_todos_grafico);
V_grafico=V_todos.*index;    %m/s2
Eta_RTM_todos_raios(1,i)=sum(V_grafico,"omitnan")./gama+Eta_RTM_HC_Omang;   % mGal
...
figure(1);
plot(radius_grafico,Eta_RTM_todos_raios);
...
Eta_RTM_todos_raios=Eta_RTM_todos_raios';