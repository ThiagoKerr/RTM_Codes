...
radius_metros_intervalo=1000;
radius_metros_total = radius_metros;
...
if strcmp('sim',modelo_densidade)
densidade_fatia=densidade(:); densidade_fatia=densidade_fatia.*index; densidade_fatia(isnan(densidade_fatia))=[];
Vz(1:length(xx1),coluna)=Prism_g_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,densidade_fatia);  % m/s2
else
Vz(1:length(xx1),coluna)=Prism_g_nosum(xx1,xx2,yy1,yy2,zz1,zz2,long,lat,r,rho);  % m/s2
end
end
...
dg_RTM_prisma_raios=cumsum((sum(Vz,"omitnan")).*1D5)+dg_RTM_HC;   % m
