...
g=polyhedron_full(size1,x_poliedro,y_poliedro,z_poliedro);
...
Vz_poliedro(1:size1,coluna)=G.*densidade_o_poliedro_c.*temp_Vz;
else
Vz_poliedro(1:size1,coluna)=G.*rho.*temp_Vz;
end
...
dg_RTM_poliedro=(sum(Vz_poliedro(:),"omitnan")).*1D5+dg_RTM_HC   % m
toc

% Distúrbio da gravidade por fatias de raios para geração de gráfico
dg_RTM_poliedro_raios=cumsum((sum(Vz_poliedro,"omitnan")).*1D5)+dg_RTM_HC;   % m
radius_grafico=1:radius_metros_total/1000;

%PEGANDO SÓ VALORES DE 1 EM 1KM ATÉ 210KM
j=1;
for i=2:2:420
    dg_RTM_poliedro_raios1km(j) = dg_RTM_poliedro_raios(i);
    j=j+1;
end
dg_RTM_poliedro_raios1km = dg_RTM_poliedro_raios1km';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Geração de imagens e gráficos %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(1);
plot(radius_grafico,dg_RTM_poliedro_raios1km);
...
dg_RTM_poliedro_raios=dg_RTM_poliedro_raios';